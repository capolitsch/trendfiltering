...
mask.intervals <- function(df, min.mask.width = 20){
  ...
  itr <- 1
  diffs <- sapply(X = 2:length(masked), 
  ...
  if ( itr > 1 ){
    inds <- unlist(lapply(X = 1:length(masked.intervals), 
                          FUN = function(X) length(masked.intervals[[X]]) > min.mask.width
    )
    )
    if ( length(masked.intervals[inds]) > 0 ){
      masked.intervals <- lapply(X = 1:length(masked.intervals[inds]), 
                                 FUN = function(X) masked.intervals[inds][[X]]
      )
    }else{
  ...
  if ( length(masked.intervals) > 0 ){
    inds <- c(df %>% pull(wavelength) %>% min - 1, 
              sapply(X = 1:length(masked.intervals), 
                     FUN = function(X) min(masked.intervals[[X]])), 
  ...
  good.wavelength.intervals <- 
    lapply(X = 1:(length(inds)-1), FUN = function(X) (inds[X] + 1):(inds[X+1]-1))
  ...
  df <- df %>% filter(mask == 0) %>% select(-mask)
